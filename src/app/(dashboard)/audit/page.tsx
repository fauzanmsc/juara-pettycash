...
const getIconAndColorForModule = (modul: string, aktivitas: string) => {
  ...
  if (a.includes("login") || m.includes("auth")) {
    return { icon: User, color: "text-purple-500", bg: "bg-purple-50 dark:bg-purple-500/20" };
  }
  if (m.includes("pengajuan")) {
    return { icon: FileText, color: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-500/20" };
  }
  if (m.includes("approval") || a.includes("approve") || a.includes("setuju")) {
    return { icon: ShieldCheck, color: "text-green-500", bg: "bg-green-50 dark:bg-green-500/20" };
  }
  if (m.includes("setting") || m.includes("pengaturan")) {
    return { icon: Settings, color: "text-orange-500", bg: "bg-orange-50 dark:bg-orange-500/20" };
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-[#151921] p-6 rounded-2xl border border-slate-200 dark:border-slate-800/60 shadow-sm">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Lacak semua log aktivitas sistem dan jejak audit (Log Histori)</p>
        </div>
      </div>
      ...
      <Card className="bg-white dark:bg-[#151921] border-slate-200 dark:border-slate-800/60 soft-shadow rounded-2xl p-6 md:p-8">
        <div className="flex items-center justify-between mb-8 border-b border-slate-100 dark:border-slate-800/60 pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-800 dark:text-white">Aktivitas Terbaru</h2>
          </div>
        </div>

        <div className="relative border-l-2 border-slate-100 dark:border-slate-800/60 ml-4 space-y-8 pb-4">
          {isLoading ? (
            Array.from({ length: 4 }).map((_, idx) => (
              <div key={idx} className="relative pl-8">
                <div className="absolute -left-[17px] top-1 w-8 h-8 rounded-full border-4 border-white dark:border-[#151921] bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
            ...
            sortedLogs.map((log: any) => {
              ...
              return (
                <div key={log.ID_Log} className="relative pl-8 group">
                  {/* Timeline dot */}
                  <div className={`absolute -left-[17px] top-1 w-8 h-8 rounded-full border-4 border-white dark:border-[#151921] flex items-center justify-center transition-transform group-hover:scale-110 ${bg}`}>
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-xs font-medium bg-slate-100 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 px-2.5 py-0.5 rounded-md border border-slate-200/50 dark:border-slate-700/50">
                          {log.Modul}
                        </span>
                      </div>
        ...
        {sortedLogs.length > 0 && (
          <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800/60 text-center">
            <Button variant="outline" className="text-slate-600 dark:text-slate-300 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
              Muat Lebih Banyak <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
          </div>
        )}
      </Card>

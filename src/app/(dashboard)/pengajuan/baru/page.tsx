...
export default function BaruPengajuanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) return alert("Anda harus login untuk membuat pengajuan.");
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4 bg-white dark:bg-[#151921] p-6 rounded-2xl border border-slate-200 dark:border-slate-800/60 shadow-sm">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Buat Pengajuan Baru</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Isi detail kebutuhan dana kas kecil</p>
        </div>
      </div>
      ...
      <Card className="bg-white dark:bg-[#151921] border-slate-200 dark:border-slate-800/60 soft-shadow rounded-2xl p-6 md:p-8">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-2">
            <Label htmlFor="desc" className="text-slate-700 dark:text-slate-300 font-semibold">Keterangan / Tujuan <span className="text-red-500">*</span></Label>
            <Textarea 
              id="desc" 
          ...
          <div className="space-y-2">
            ...
            <div className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center bg-slate-50/50 dark:bg-[#0D0F14]/50 hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors cursor-pointer group">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">PDF, JPG, atau PNG (Maks. 5MB)</p>
            </div>
          </div>
